...
class graph {
private:
    static std::unordered_map<long int, node*> nodes;
    static int instance_count;

    int graph_id {};

    // Getters:
    static node* get_node_by_id (const long int id);
    static std::vector<long int> get_node_ids ();
...
public:
    ...
    // Regular methods:
    std::vector<long int> compute_shortest_path (const long int source_node_id, const long int destination_node_id);
...
struct priorityq_entry {
    const long int current_node_id;
    const long int parent_node_id;
    std::shared_ptr<cost> path_cost;

    priorityq_entry(const long int c_node_id, const long int p_node_id, std::shared_ptr<cost> p_cost) 
