...
long int node::node_count = 0;

node::node() : longitude(0.0f), lattitude(0.0f), id(++node_count) {             // Default constructor
    graph::add_to_graph(this);
}

node::node (double lattitude_val, double longitude_val) {                       // Optional Value Constructor
    // Constructor chaining
    node(++node_count, lattitude_val, longitude_val);
}

node::node (long int id_value, double lattitude_val, double longitude_val) 
    : id(id_value), lattitude(lattitude_val), longitude(longitude_val) {        // All Parameters Constructor
    graph::add_to_graph(this);
}

path* node::calculate_path(const long int neighbour_id) {
...
bool node::add_neighbour (long int neighbour_id) {
    if (neighbours.find(neighbour_id) != neighbours.end())
        return false;
    // Null check
...
bool node::add_neighbour(long int neighbour_id, const path &path_to_neighbour) {
    if (neighbours.find(neighbour_id) != neighbours.end())
        return false;
    neighbours[neighbour_id] = path_to_neighbour;
    return true;
...
std::vector<long int> node::get_neighbour_ids () {
    std::vector<long int> node_neighbours{};
    for (auto &neighbour : neighbours) {
...
bool node::add_heuristic (const int graph_id, const long int destination_node_id) {
